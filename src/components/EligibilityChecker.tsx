...
function getResult(answers: Record<string, string>): Result {
  ...
  if (received === 'seizure_salary') {
    return {
      ...
      emoji: '💼',
      urgency: 'high',
      description: 'סכום מוגן לפי הרכב המשפחה חייב להישאר בידך. שכר שנמוך מהסכום המוגן — אסור לעקל בכלל. אפשר לבקש הפחתה.',
      steps: [
        'בדוק שנשאר בידך לפחות הסכום המוגן לפי הרכב משפחתך (2,596–5,289 ₪ נטו, 2026)',
        'אם השכר שלך נמוך מהסכום המוגן — העיקול אינו חוקי',
        'הגש בקשה לצו תשלומים או להפחתת העיקול',
      ],
      guides: [
  ...
  if (received === 'letter') {
    return {
      title: 'קיבלת מכתב — יש לך לרוב 30 יום',
      emoji: '✉️',
      urgency: 'high',
      description: 'מכתב מהוצאה לפועל נותן לך לרוב 30 יום להגיב. אל תתעלם — זה הזמן לפעול ולמנוע עיקולים.',
      steps: [
        'קרא את המכתב ומצא: מספר תיק, שם הנושה, סכום',
        'בדוק אם החוב אמיתי ואם הסכום נכון',
        'הגש בקשה לצו תשלומים בתוך 30 יום',
      ],
      guides: [
  ...
  return {
    ...
    steps: [
      ...
      'מלא טופס בקשה לצו חיוב בתשלומים (טופס 233) + שאלון (טופס 529)',
